...
make_bw_plot<- function(fagregated, By,titl){
  d = fagregated;
  if (By == 'Faculties' )
    d = subset(d, Campuses = 'JAMSHORO')
    
    
  if (By == 'Campuses')
    {d <- d[grep('CAMPUS',d$Campuses),]
    d$Campuses <- factor(d$Campuses, exclude = 'JAMSHORO')
  }
  ...
  if (By == 'Campuses')
    print(bwplot(data=d, Means ~ reorder (d$Indicators,d$Means) | d$Campuses,
           scales= list(x=list(rot=90)),
           main=paste('Boxplots of', titl,'\n  ','in various Campuses')))    
    ...
    {
      #old_d2 = subset_by(d, 'Jamshoro',faculty,lev)
      d2 <- subset(d, Campuses = 'JAMSHORO')
...
make_correlogram <- function(fmelted, By,titl){
  ...
  if (By == 'Campuses'){
    ...
    corr = round(cor(d_casted[,-1]),2)
    heading = paste(titl,"\nThe Correlogram Matrix of Indicators of additional Campuses Overall"  )
  }
...
sceq_correlogram <- function(sceq, By){
  ...
  if (By == 'Faculties' ){
    ...
    heading = "Students Course Evaluation 2019-21\nThe Correlogram Matrix of Indicators of various Faculties in Jamshoro"
 ...
 # percent1[]
  ...
  for (i in 1:nrow(u)){
    data <- subset(percent1[,1:6], percent1$pasted == u$pasted[i])
          ...
          jpeg(filename = paste0('../output',filname,'.jpg'),width = 600)
